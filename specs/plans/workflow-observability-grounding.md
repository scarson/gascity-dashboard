# Grounding: Workflow Observability in the Gas City Dashboard
...
1. **Workflows** (`routes/Workflows.tsx`, `components/workflow/{WorkflowMap,LaneCard}.tsx`,
   `snapshot/collectors/workflows.ts`). Each workflow root = one lane row: phase label,
   ...
   `WorkflowSummary`/`WorkflowLane` in `shared/src/snapshot/types.ts`.
...
- Workflows and Triage both show "active work" but in different shapes (bead-hierarchy lanes
  vs GitHub priority queue) with independently-computed notions of "active/stuck."
- No drill-in from Workflows → run/bead/session detail.
- No cross-link Triage ↔ Workflows (an issue's workflow membership is invisible).
- Agent *progress* (what a session is doing now) is invisible from Workflows/Triage; you
  must visit `/agents/{session}` separately.
- Activity is uncorrelated with workflows and not live.
- "Is anything off?" has **no single home** — the operator must scan three surfaces.

## The csells asset (`csells/workflows-formula-runs` branch)

A proposed **run-detail graph.v2 view** (plan: `specs/plans/workflow-run-detail-plan.md`,
mockup SVG: `specs/plans/assets/workflow-run-detail-graphv2-adopt-pr.svg`). One run = a
...
selected node). Backend enrichment (`workflows/enrich.ts`, `workflows/diff.ts`,
`routes/session-stream.ts`) ports the gasworks-gui `WorkflowSnapshot` + `display_graph`
...
- **Cross-cutting engine (the real unifier):** a single shared **health derivation** —
  **#3 Staleness / time-since-last-progress**, **#22 Epistemic uncertainty/confidence**
  ...
  ONCE and consumed by all three layers, so Workflows/Triage/Activity stop independently
  re-deriving "active/stuck."

**Reconciliation of the three existing surfaces under this model:**
- **Workflows** lane strip → a thin INDEX whose rows finally drill into the csells
...
4. Does collapsing Triage → Needs-You lose the maintainer's existing triage workflow?
